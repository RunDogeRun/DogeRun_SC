...
contract RunDoge is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Doge Run";
    string constant _symbol = "DRun";
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBGame = amountBNB.mul(gameFee).div(totalBNBFee);
        uint256 amountBNBTeam = amountBNB.mul(buyBackFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(gameFeeReceiver).call{value: amountBNBGame, gas: 30000}("");
        (tmpSuccess,) = payable(buyBackFeeReceiver).call{value: amountBNBTeam, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _gameFee, uint256 _buyBackFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        gameFee = _gameFee;
        buyBackFee = _buyBackFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_gameFee).add(_buyBackFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/4);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _gameFeeReceiver, address _buyBackFeeReceiver) external authorized {
